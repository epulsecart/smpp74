...
class SmppClient
{
	...
	protected function _bind($login, $pass, $command_id)
	{
        ...
        info ("sending command to bind ");
		$response=$this->sendCommand($command_id,$pduBody);
        info ("the response status is ");
	...
	{
		if (!$this->transport->isOpen()) return false;
        info ("the connection is still open ");
...
class SmppAddress
{
	 ...
	 * @throws InvalidArgumentException
	 */
	public function __construct($value,$ton=SMPP::TON_INTERNATIONAL,$npi=SMPP::NPI_UNKNOWN)
	{
